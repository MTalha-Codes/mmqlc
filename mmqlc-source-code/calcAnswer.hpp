...
public:
   answer_calc(std::vector<std::tuple<std::string ,double ,double>> const &parsedReal , std::vector<std::tuple<std::string,std::complex<double>,std::complex<double>>> const &parsedComplex)
   {
   ...
   std::vector<std::string> calculateAnswers()
   {
        for(const auto &parsedToken : realNums_parsed)
        {
            ...
            auto process_real_double = [this](const std::string &query, const double &fOperand, const double &sOperand) {
                if(query == "ADD")
                {
        ...
        for(const auto& parsedToken : cmplxNums_parsed)
        {
            ...
            auto process_complex_double = [this](const std::string &query, const std::complex<double> &fOperand, const std::complex<double> &sOperand) {
                if (query == "CMPLX_ADD")
                {
