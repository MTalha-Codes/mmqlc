...
inline std::vector<std::tuple<std::string, std::string, std::string>> tokenize(std::vector<std::string> &queries) {
    ...
    auto processReal = [&tokens](const std::string &query) {
       const  std::unordered_set<std::string> SingleOperand_Queries = {
        ...
        const std::regex singlePattern_noVariable(R"((\w+)\s(-?\d+(\.\d+)?))"); // queries called directly with numeral inputs
        const std::regex doublePattern_noVariable(R"((\w+)\s(-?\d+(\.\d+)?),(-?\d+(\.\d+)?))"); // queries called directly with numeral inputs
       if (std::smatch matches; std::regex_match(query, matches, singlePattern_noVariable) && matches.size() >= 3) {
          ...
          std::string  keyWord = matches[1].str();
           std::string   firstOperand = matches[2].str();
            std::string secondOperand = matches[4].str();
    ...
    auto processComplex = [&tokens](const std::string &query) {
        ...
        const std::regex singlePattern_noVariable(R"((\w+)\s(-?\d+(\.\d+)?[+-]-?\d+(\.\d+)?i))");
        const std::regex doublePattern_noVariable(R"((\w+)\s(-?\d+(\.\d+)?[+-]-?\d+(\.\d+)?i),(-?\d+(\.\d+)?[+-]-?\d+(\.\d+)?i))");
        if (std::smatch matches; std::regex_match(query, matches, singlePattern_noVariable) && matches.size() >= 3) {
            if (std::string keyWord = matches[1].str(); SingleOperand_Queries.contains(keyWord)) {
                std::string firstOperand = matches[2].str();
                tokens.emplace_back(keyWord, firstOperand, "");
        ...
        } else if (std::regex_match(query, matches, doublePattern_noVariable) && matches.size() >= 5) {
           std::string  keyWord = matches[1].str();
          std::string   firstOperand = matches[2].str();
           std::string secondOperand = matches[5].str();
            tokens.emplace_back(keyWord, firstOperand, secondOperand);
            return true;
        }
        return false;
